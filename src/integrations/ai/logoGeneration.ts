...
import { LogoGenerationError } from "./types";
...
export async function generateLogoConcepts(
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      const prompt = `Create a minimalist, professional logo for ${params.brandName}, a ${params.industry} company. 
        Style: ${params.aestheticPreferences?.join(", ") || "modern and professional"}
        Colors: ${params.colorPreferences?.join(", ") || "brand appropriate"}`;
      ...
      logos.push({
        id: `logo-${Date.now()}-${i}`,
        url: data.image_url, // Adjust based on actual response format
        prompt,
        selected: false
      });
    }
